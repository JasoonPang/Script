  ...
  if ($nobyda.isRequest) {
    ...
    if (cookie.includes("P00001") && cookie.includes("P00003") && cookie.includes("__dfp=")) {
      ...
      for (let i = 0; i < tasks.length; i++) {
        console.log('任务状态：' + tasks[i].name + ':' + tasks[i].status + ':' + tasks[i].taskCode)
        if (![1, 4].includes(tasks[i].status) && !['观看视频30分钟', '开通自动续费'].includes(tasks[i].name)) { //0：待领取 1：已完成 2：未开始 4：进行中
